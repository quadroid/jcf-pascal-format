...
const
  CODEFORMAT_SETTINGS_SECTION = 'CodeFormatSettings';

function GetDefaultSettingsFileName: string;
begin
  Result := IncludeTrailingPathDelimiter(GetApplicationFolder) + 'pascal-format.cfg';
end;

procedure TFormatSettings.Read;
begin
  // use the Settings File if it exists
  ReadFromFile(GetDefaultSettingsFileName, False);
end;
...
begin
  ...
  if FileExists(GetDefaultSettingsFileName) and FileIsReadOnly(GetDefaultSettingsFileName) then
  begin
    { fail quietly? }
  ...
  try
    ...
    lcFile := TSettingsStreamOutput.Create(GetDefaultSettingsFileName);
    try
      ToStream(lcFile);
