...
begin
  ...
  Result := String(lsContents8bit);
end;


function ReadUtf8File(const pcFileStream: TFileStream): String;
var
  //bomBuf: array[0..2] of Byte;
  liBytesRemaining: integer;
  lsContents: UTF8String;
begin
  liBytesRemaining := pcFileStream.Size - pcFileStream.Position;
  {if liBytesRemaining >= length(bomBuf) then
  begin
    pcFileStream.ReadBuffer(bomBuf[0], length(bomBuf));
    if not ((bomBuf[0] = $ef) and (bomBuf[1] = $bb) and (bomBuf[2] = $bf)) then
      pcFileStream.Seek(0, soFromBeginning);
    liBytesRemaining := pcFileStream.Size - pcFileStream.Position;
  end;}
  SetLength(lsContents, liBytesRemaining);
  pcFileStream.ReadBuffer(lsContents[1], liBytesRemaining);
  Result := String(lsContents);
end;
...
begin
  lsContents := UTF8String(psContents);
  Len := Length(lsContents);
  // write the BOM
 ...
 begin
  ...
  try
   ...
   case peContentType of
     e8Bit:
     begin
       // FIX: always write back as UTF-8
       {Write8BitFile}WriteUtf8File(fs, psContents);
     end;
