...
const
  ...
  // fix: needed for proper line break inside `end <comment> else`
  NotSolidTokensButComment: TTokenTypeSet =
...
begin
  ...
  for liMapItemLoop := liStart to liEnd do
    ...
    end{ 'a simplified version of Boyer-Moore comparison technique' }{ matched first and last and uppercase }; { for loop through the array }
end;
...
begin
  ...
  if peTokenType = ttIdentifier then
    Result := wtIdentifier// identifier not in the list as it has no fixed text
