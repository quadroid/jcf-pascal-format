...
uses
  ...
  JcfMiscFunctions, JcfSettings, JcfUnicodeFiles;
...
begin
  ...
  begin
    WriteLn(ErrOutput, 'File: ' + psInputFileName + ' cannot be processed as it is read only');
    exit;
  end;
    ...
    if lbFileIsChanged and FileExists(lsOut) then
      if not DeleteFile(lsOut) then
        raise Exception.Create('TFileConverter.ProcessFile: ' +
          'Failed to delete ' + lsOutType + ' file ' + lsOut)//if YesAll then
      //  wRes := mrYes
      //else
      //begin
