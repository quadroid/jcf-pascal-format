  ...
  {$ENDIF}
    ...
    try   { finally free the list  }
      ...
      if fbConvertError then
        lcTokenList.Clear{ if there was a parse error, the rest of the unit was not parsed
...
begin
  ...
  if (pcRoot is TSourceToken) then
    ...
    for liLoop := 0 to pcRoot.ChildNodeCount - 1 do
      CollectOutput(pcRoot.ChildNodes[liLoop])// recurse, write out all child nodes
  ;
end;
