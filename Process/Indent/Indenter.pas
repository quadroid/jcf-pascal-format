  ...
  begin
    ...
    if not HasPreceedingSolidToken(pt) then
      if pt.HasParentNode(nBlock) then
        ...
        Result := FormatSettings.Indent.KeepCommentsWithCodeElsewhere{ if it's a single line comment, with nothing on the line before it
...
begin
  ...
  begin
    ...
    // fix: decrease ident for interface guid
    if not FormatSettings.Indent.IndentInterfaceGuid then
      if (pt.TokenType = ttOpenSquareBracket) and pt.HasParentNode(nInterfaceType, 2) then
        dec(liIndentCount);
    ...
    // fix: ability to not indent long method parameters, e.g.:
    // procedure DoIt (param1: string; param2: integer
    // ; param3: single); <-- keep the semicolon unindented
    if pt.Nestings.GetLevel(nlRoundBracket) > 0 then
      if not FormatSettings.Indent.IndentMethodParams then
        Dec(liIndentCount);
  end
  ...
  begin
    ...
    if pt.Nestings.GetLevel(nlRoundBracket) > 0 then
      if not FormatSettings.Indent.IndentMethodParams then
        liIndentCount := Max(liIndentCount - 1, 0);
  end
  else
  begin
    { this section is for
    ...
    if pt.HasParentNode(nFormalParams) then
      if FormatSettings.Indent.IndentMethodParams then
        Inc(liIndentCount);
    ...
    if pt.HasParentNode([nLoopHeaderExpr, nBlockHeaderExpr]) then
      lbHasIndentedRunOnLine := True;//dec(liIndentCount);

    if pt.Nestings.GetLevel(nlCaseSelector) > 0 then
      if FormatSettings.Indent.IndentCaseLabels then
      begin
      ...
      begin
        // fix: case labels on the same level as `case` statement
        liIndentCount := Max(liIndentCount + pt.Nestings.GetLevel(nlCaseSelector) - 1, 0);
    ...
    begin
      ...
      if (pt <> lcChild) then
      begin
        // fix: apparently `IsInProcedureParams()` fails in cases like:
        //   param1, param2 <-- affected
        //   ); <-- not caught, because at this point `nlRoundBracket` is zero?
        Inc(liIndentCount);
        lbHasIndentedRunOnLine := True;
  ...
  if FormatSettings.Indent.IndentBeginEnd then
    if (pt.TokenType in [ttTry, ttExcept, ttFinally, ttBegin, ttEnd]) and InStatements(pt) then
      if not pt.HasParentNode(nBlock, 2) then
        Result := Result + FormatSettings.Indent.IndentBeginEndSpaces;// filter out the begin/end that starts and ends a procedure
end;
