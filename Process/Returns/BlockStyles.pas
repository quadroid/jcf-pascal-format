...
begin
  ...
  lcNextToken := pt.NextSolidTokenOrComment; // fix: don't touch anything with comments in-between
  ...
  begin
    ...
    begin
      ...
      if pt.HasParentNode (BlockHeadings, 3)
        {and (lcNextToken.NextSolidToken.TokenType = ttIf)} then
        Result := eLeave;
    end;
  end
  ...
  begin
    ...
    begin
      ...
      if lcNextToken.TokenType = ttBegin then
        ...
        Result := FormatSettings.Returns.LabelStyle{ otherwise, is there a begin next? };
  end
  ...
  begin
    ...
    begin
      ...
      Result := FormatSettings.Returns.ElseIfStyle{ though else normally starts a block,
      ...
      Result := FormatSettings.Returns.ElseBeginStyle // fix: respect the `else begin` style
    else
  ...
  if (lcNextToken.TokenType = ttBegin) or (lcNextToken.TokenType = ttTry) or
    ...
    Result := FormatSettings.Returns.BlockStyle{ otherwise, is there a begin next? };
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (lcNextReturn <> nil) and (lcNextReturn.TokenType = ttReturn) then
      begin
        // fix: if there is a comment before the return, don't remove the return
        lcNextComment := lcSourceToken.NextTokenWithExclusions([ttWhiteSpace]);
        if lcNextComment.CommentStyle = eNotAComment then
        begin
          // turn to space
