...
begin
  ...
  begin
    ...
    if IsUnaryOperator(pcToken) then
      piScoreAfter := BAD4{ do not break between unary operator and operand }
    else
    begin
    ...
    ttOpenBracket:
      ...
      begin
        ...
        piScoreBefore := BAD2;
        piScoreAfter  := GOOD1;
      end;{ if this is a fn def or call, break after. }
      ...
      begin
        ...
        if lcNext <> nil then
          if lcNext.TokenType in Operators then
            piScoreAfter := BAD1{ operator next? want to break after it instead of after the ')'
                (for e.g after the + in 'a := (x - y) + z; }
          else if (lcNext.TokenType in [ttCloseBracket, ttCloseSquareBracket]) then
            piScoreAfter := BAD2{ more close brackets coming? break after them instead }
          else
          begin
  ...
  begin
    ...
    ttExternal:
      ...
      begin
        ...
        piScoreAfter  := BAD1;
      end;// the function directive external is followed by text
    ...
    ttDefault:
      ...
      begin
        piScoreBefore := BAD1;
        piScoreAfter  := GOOD1;
      end;{ default attr in a property is a bad thing to break before }
    end;
