...
begin
  ...
  begin
    ...
    if (pt.TokenType = ttDefault) and pt.HasParentNode(nPropertySpecifier) then
      ...
      begin
        Result := True;
        exit;
      end{ use the persence of semicolon to distinguish
  ...
  if lcSetReturns.RemoveExpressionReturns and pt.HasParentNode(nExpression) then
    ...
    begin
      Result := True;
      exit;
    end{ can have a block that ends in expression without a semicolon, eg Don't remove return here:
      begin
        a := a + 2
      end;    };
...
begin
  ...
  if not (lcSourceToken.TokenType in [ttWhiteSpace, ttReturn]) then
    fcLastSolidToken := lcSourceToken{ store for next time };
end;
